...
fig_names_df1 <- data.frame("name"=c("a","b","c","d","e","f","g"),
                            "week"=unique(df_to_plot_prep$weeks_post_infection))


#### list of plots with each variable separately
list_of_plots <- list()
for (i in fig_names_df1$week){
 
  #### get stats formatting
#  https://www.datanovia.com/en/blog/how-to-add-p-values-onto-a-grouped-ggplot-using-the-ggpubr-r-package/
 
  curr_fig_name <- fig_names_df1$name[which(fig_names_df1$week == i)]
        cat("### ",paste0(fig_name_to_use,curr_fig_name),"\n")
      cat('\n')
  ...
  scale_fill_manual(values = c(Control = "black",
                                Alum = "blue",
                                ALFQA = "red"))+
    geom_beeswarm(aes(x = Group, y = !!rlang::sym(paste0("VL_week",i)), group = Group, fill = Group), dodge.width = dodge_to_use, cex = 3, size = 3, shape = 21, color = "black") + 
  ...
  curr_stat_test,  label = "p", tip.length = 0
    ...
    stat_summary(
    fun = "median",        #argument updated in new version.
    geom = "point",
    col = "grey30",
    size = 12,
    shape = 95,
...
scatter_df <- data.frame("name"=c("Fig2d",
                                  "Fig2i"),
                         "y"=c("Plasma_V2specific_ADCC_Killing__WkPost",
                               "Plasma_ADNP_SIVgp120dV1__Change"),
                         "trans_y"=c("identity",
                                     "identity"),
                         "dataset"=c("other",
...
```{r AssociationWithTOA_prep___other}
 ...
 corr_timepoints <- c("WkPost","Change") ### not looking at baseline associations
...
scatter_df <- data.frame("name"=c("Fig3c",
                         ...
                         "y"=c("RectMucosa_DC10__Change",
                               "RectMucosa_NKp44__Change",
                               "RectMucosa_NKp44_gp120_IL17__Change"),
                         "trans_y"=c("identity",
                                     "identity",
                                     "identity"),
                         "dataset"=c("other",
...
```{r Fig6e_summary, results='asis'}
fig_name_to_use <- "Fig6e"


#### collect all MW
combo_stats <- list_of_stats[["Fig2l_MW_2group"]] %>% ## first from plasma
  ## then from RM
  dplyr::full_join(list_of_stats[["Fig3j_MW_2group"]],
                   by = colnames(.)) %>%

  ## then for olink, keeps the real wk
  dplyr::full_join(list_of_stats[["Fig5a_MW_2group"]],
                   by = colnames(.)) %>%

      ### should have all the var names, but need to make generic wk
  dplyr::mutate(rowname = ifelse(grepl("Wk",rowname),
                                 paste0(str_before_last(rowname,"__Wk"),"__WkPost"),
                                 rowname)) %>%


#combo_stats2 <- 
dplyr::left_join(
    list_of_stats[["Fig2l_vsTOA"]] %>%
                     full_join(list_of_stats[["Fig3j_vsTOA"]], by = c("assay1","assay2","R_ALFQA","p_ALFQA")) %>%
                     full_join(list_of_stats[["Fig5a_vsTOA"]], by = c("assay1","assay2","R_ALFQA","p_ALFQA","R_ALUM","p_ALUM")) %>%
    dplyr::rename(rowname = assay1) %>%
    dplyr::select(-assay2),
  by = "rowname") %>%

  ### now trim to p < 0.05 for MW OR TOA p< 0.05 for at least one
  dplyr::filter(p_raw < 0.05 | p_ALFQA < 0.05 | p_ALUM < 0.05) %>%
  ## add helpful columns
  dplyr::mutate(MW_dir = ifelse(p_raw < 0.05,
                                ifelse(est > 0, "up","down"),
                                "not_sig"),
                vsTOA_ALFQA_dir = ifelse(p_ALFQA < 0.05,
                                         ifelse(R_ALFQA > 0,"pos","neg"),
                                         "not_sig"),
                vsTOA_ALUM_dir = ifelse(p_ALUM < 0.05,
                                         ifelse(R_ALUM > 0,"pos","neg"),
                                         "not_sig")) %>%
  dplyr::rename(MW_p_raw = p_raw, MW_p_adj=p_adj, MW_est = est) %>%
  dplyr::relocate(rowname,MW_dir,vsTOA_ALFQA_dir,vsTOA_ALUM_dir)



 to_print <- combo_stats %>%
 ...
 # 
      ...
      cat(' \n \n') ### this is the key!
    #### also save to stats & plotted
      list_of_plotted[[paste0(fig_name_to_use)]] <- combo_stats
      list_of_stats[[paste0(fig_name_to_use)]] <- combo_stats
```
